using Business.Interfaces.Constants;
using Business.Interfaces.Model;
using Business.Interfaces.WebModels.Subscriptions;
using CQRS.Behaviours;

namespace CQRS.Subscriptions.GetPublicSubscriptionPlans;

public sealed record GetPublicSubscriptionPlansQuery(Guid TenantId)
    : IRequestQuery<IEnumerable<SubscriptionPlanInfoWeb>>, IAuthorizableRequest, IBypassSubscriptionCheck
{
    public string PermissionCode => PermissionCodes.TenantView;
    public ResourceRef GetResource() => new(TenantId: TenantId);
}
