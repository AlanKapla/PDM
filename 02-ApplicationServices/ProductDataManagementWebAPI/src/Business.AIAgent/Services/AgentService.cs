...
public sealed class AgentService : IAgentService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // If image content is provided, use vision-enabled execution
            if (request.ImageContent != null && request.ImageContent.Length > 0)
            {
                _logger.LogInformation(
                    "Processing request with image content (size: {Size} bytes)",
                    request.ImageContent.Length);

                result = await _orchestrator.ExecutePromptWithImageAsync(
                    request.Prompt,
