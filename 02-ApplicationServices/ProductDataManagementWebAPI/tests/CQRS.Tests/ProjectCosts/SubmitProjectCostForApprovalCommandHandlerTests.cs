...
public sealed class SubmitProjectCostForApprovalCommandHandlerTests
{
    ...
    private readonly Mock<ILogger<SubmitProjectCostForApprovalCommandHandler>> _loggerMock = new();
    private readonly SubmitProjectCostForApprovalCommandHandler _handler;

    private static readonly Guid UserId = Guid.NewGuid();
    private static readonly Guid TenantId = Guid.NewGuid();
    private static readonly Guid ProjectId = Guid.NewGuid();

    public SubmitProjectCostForApprovalCommandHandlerTests()
    {
        _currentUserMock.Setup(u => u.Id).Returns(UserId);
        _currentUserMock.Setup(u => u.FullName).Returns("Test User");

        _handler = new SubmitProjectCostForApprovalCommandHandler(
    ...
    private static ProjectCost BuildProjectCost(Guid costId, CostApprovalStatus status = CostApprovalStatus.Draft)
    {
    ...
    private static SubmitProjectCostForApprovalCommand BuildCommand(Guid costId)
    {
        return new SubmitProjectCostForApprovalCommand
        {
            TenantId = TenantId,
            ProjectId = ProjectId,
            CostId = costId
    ...
    [Fact]
    public async Task Handle_WhenCostExistsAndUserHasWriteAccess_SubmitsCostAndReturnsWeb()
    {
        // Arrange
        Guid costId = Guid.NewGuid();
        ProjectCost projectCost = BuildProjectCost(costId, CostApprovalStatus.Draft);
        SubmitProjectCostForApprovalCommand command = BuildCommand(costId);
        ...
        result.ApprovalStatus.Should().Be(CostApprovalStatus.PendingApproval);
        projectCost.ApprovalStatus.Should().Be(CostApprovalStatus.PendingApproval);
    ...
    {
        // Arrange
        SubmitProjectCostForApprovalCommand command = BuildCommand(Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_WhenCostIsNotDraft_ThrowsValidationApiException()
    {
        // Arrange
        Guid costId = Guid.NewGuid();
